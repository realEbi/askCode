...
local T = new_set({
  hooks = {
    pre_case = function()
      ...
      child.lua([[Agent = require('askCode.agents.opencode')]])
    end,
...
T["prepare_command"]["should contain opencode run --format json and temp file"] = function()
  local command = child.lua_get([[Agent.prepare_command("test prompt")]])
  eq(type(command), "string")
  eq(command:match("opencode run %-%-format json") ~= nil, true)
...
T["parse_response"]["should extract text from json event stream"] = function()
  local json_stream = table.concat({
    '{"type":"step_start","part":{}}',
    '{"type":"text","part":{"type":"text","text":"Hello, "}}',
    '{"type":"text","part":{"type":"text","text":"world!"}}',
    '{"type":"step_finish","part":{}}',
  }, "\n")
  local result = child.lua_get(string.format([[Agent.parse_response(%q)]], json_stream))
...
T["parse_response"]["should return nil when no text events present"] = function()
  local json_stream = '{"type":"step_start","part":{}}\n{"type":"step_finish","part":{}}'
  local result = child.lua_get(string.format([[Agent.parse_response(%q)]], json_stream))
  eq(result, vim.NIL)
end
...
T["ask"]["should return parsed response from command"] = function()
  local mock_output = '{"type":"text","part":{"type":"text","text":"mocked opencode response"}}'
  child.lua(string.format([[
    local original_popen = io.popen
    io.popen = function()
      return {
        read = function() return %q end,
        close = function() end,
      }
    end
  ]], mock_output))

  eq(child.lua_get([[Agent.ask("test prompt")]]), "mocked opencode response")
